...
class VentasService {
    ...
    return _firestore
      ...
      return snapshot.docs.map((doc) {
        ...
        return ProductoInventario(
          ...
          nombre: data['nombre'] ?? '',
          imei: data['imei'] ?? '',
          precio: (data['total'] ?? 0).toDouble(), // Usamos 'total' como precio
          stock: 1, // Por defecto 1, ya que cada IMEI es único
  ...
  Future<Venta?> getVentaPorNumero(String numeroVenta) async {
    try {
      final snapshot = await _firestore
          .collection(_ventasCollection)
          .where('numeroVenta', isEqualTo: numeroVenta)
          .limit(1)
          .get();
      ...
      print('❌ Error al obtener venta por número: $e');
